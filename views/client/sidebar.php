                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="dropdown topbar-head-dropdown ms-1 header-item">
                            <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
                                ...
                                <span
                                    class="position-absolute topbar-badge fs-10 translate-middle badge rounded-pill bg-info"><?=format_cash(isset($getUser) ? $CMSNT->get_row("SELECT COUNT(id) FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ")['COUNT(id)'] : 0);?></span>
                            </button>
                            <div class="dropdown-menu dropdown-menu-xl dropdown-menu-end p-0 dropdown-menu-cart"
                                ...
                                <div class="p-3 border-top-0 border-start-0 border-end-0 border-dashed border">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-auto">
                                            <span
                                                class="badge badge-soft-warning fs-13"><span><?=format_cash(isset($getUser) ? $CMSNT->get_row("SELECT COUNT(id) FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ")['COUNT(id)'] : 0);?></span>
                                                <?=__('đơn');?></span>
                                        </div>
                                    </div>
                                </div>
                                <div data-simplebar style="max-height: 300px;">
                                    <div class="p-2">
                                        ...
                                        <?php if(isset($getUser) && $CMSNT->get_row("SELECT COUNT(id) FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ")['COUNT(id)'] == 0):?>
                                        <div class="text-center empty-cart">
                                            <div class="avatar-md mx-auto my-3">
                                                <div class="avatar-title bg-soft-info text-info fs-36 rounded-circle">
                                                    <i class='bx bx-cart'></i>
                                        ...
                                        <?php endif?>
                                        <?php if(isset($getUser) && $CMSNT->get_row("SELECT COUNT(id) FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ")['COUNT(id)'] != 0):?>
                                        <?php foreach($CMSNT->get_list(" SELECT * FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ") as $orderNav):?>
        ...
        <div class="app-menu navbar-menu">
            ...
            <div id="scrollbar">
                <div class="container-fluid">
                    ...
                    <?=__('Giảm giá');?>: <b style="color: red;"><?=isset($getUser['chietkhau']) ? $getUser['chietkhau'] : 0;?>%</b></span></li>
                        ...
                        <li class="nav-item">
                            <a class="nav-link menu-link <?=active_sidebar(['orders', 'order']);?>"
                                ...
                                <span
                                    class="badge badge-pill bg-danger"><?=format_cash(isset($getUser) ? $CMSNT->get_row("SELECT COUNT(id) FROM `orders` WHERE `status` = 0 AND `buyer` = '".$getUser['id']."' ")['COUNT(id)'] : 0);?></span>
                            </a>
                        </li>
